...
def get_min_sdk(project_dir):
    with open(os.path.join(project_dir, 'gradle', 'libs.versions.toml')) as file_handle:
        for line in file_handle:
            ...
            if len(tokens) == 3 and tokens[0] == 'android-minSdk = ':
                return int(tokens[1])

    fail('Failed to find android-minSdk')
...
def check_and_copy_prebuilt_libraries():
    ...
    try:
        ...
        target_libs_dir = os.path.join(project_dir, 'composeApp', 'src', 'androidMain', 'jniLibs')
...
for target in BUILD_TARGETS:
    ...
    target_dir = os.path.join(project_dir, 'composeApp', 'src', 'androidMain', 'jniLibs', target['jni_dir'])
