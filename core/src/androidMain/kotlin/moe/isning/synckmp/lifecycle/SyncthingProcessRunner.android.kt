 ...
 */
class AndroidSyncthingProcessRunner(
    ...
    override suspend fun stop(graceful: Boolean, killDelayMs: Long): Int? = withContext(Dispatchers.IO) {
        ...
        return@withContext try {
            p.destroy() // best effort; API 21 compatible
            if (!graceful) {
                // No destroyForcibly() pre-26; fall back to repeated destroy attempts
...
): SyncthingProcessRunner = AndroidSyncthingProcessRunner(scope, notificationSink)
