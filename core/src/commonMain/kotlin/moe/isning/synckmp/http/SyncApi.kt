...
import moe.isning.synckmp.config.*

@Target(AnnotationTarget.FUNCTION)
@Retention(AnnotationRetention.BINARY)
@RequiresOptIn
annotation class SyncExpensiveEndpoint

class SyncApi(
    private val cfg: SyncApiConfig,
    ...
    suspend fun getSystemVersion(): SystemVersion =
        client.get { apiUrl(SyncEndpoints.VERSION) }.body()

    suspend fun getSystemStatus(): SystemStatus =
        client.get { apiUrl(SyncEndpoints.SYSTEM_STATUS) }.body()

    suspend fun getConnections(): Connections =
        client.get { apiUrl(SyncEndpoints.CONNECTIONS) }.body()

    suspend fun getSystemLogLevels(): JsonObject =
        client.get { apiUrl(SyncEndpoints.SYSTEM_LOGLEVELS) }.body()

    suspend fun setSystemLogLevels(levels: Map<String, String>): Boolean {
        val response = client.post {
            apiUrl(SyncEndpoints.SYSTEM_LOGLEVELS)
            setBody(levels)
        }
    ...
    suspend fun getDiscovery(): JsonObject =
        client.get { apiUrl(SyncEndpoints.SYSTEM_DISCOVERY) }.body()

    suspend fun clearSystemError(): Boolean {
        val response = client.post { apiUrl(SyncEndpoints.SYSTEM_ERROR_CLEAR) }
        return response.status.isSuccess()
    }

    suspend fun getSystemErrors(): SystemErrorResponse =
        client.get { apiUrl(SyncEndpoints.SYSTEM_ERROR) }.body()

    suspend fun postSystemError(message: String): Boolean {
        val response = client.post {
            apiUrl(SyncEndpoints.SYSTEM_ERROR)
            setBody(message)
        }
        return response.status.isSuccess()
    }
    ...
    suspend fun getSystemLog(since: String? = null): SystemLogResponse =
        client.get {
            apiUrl(SyncEndpoints.SYSTEM_LOG)
    ...
    suspend fun getSystemLogTxt(since: String? = null): String =
        client.get {
            apiUrl(SyncEndpoints.SYSTEM_LOG_TXT)
            if (!since.isNullOrBlank()) parameter("since", since)
        }.body()

    /** 获取 requiresRestart 状态 */
    suspend fun getRequiresRestart(): Boolean =
        client.get { apiUrl(SyncEndpoints.CONFIG_RESTART_REQUIRED) }
            .body<JsonObject>()["requiresRestart"]?.jsonPrimitive?.booleanOrNull ?: false

    // Shutdown Syncthing
    suspend fun shutdown(): Boolean {
        val response = client.post { apiUrl(SyncEndpoints.SYSTEM_SHUTDOWN) }
        return response.status.isSuccess()
    }
    ...
    suspend fun getDeviceId(): String =
        client.get { apiUrl(SyncEndpoints.DEVICE_ID) }.body()

    suspend fun getReport(): ReportInfo =
        client.get { apiUrl(SyncEndpoints.REPORT) }.body()

    suspend fun getSupportBundle(): ByteArray =
        client.get { apiUrl(SyncEndpoints.DEBUG_SUPPORT) }.body()

    // ---------- Database / Folders ----------

    @SyncExpensiveEndpoint
    suspend fun getDbCompletion(folder: String, device: String? = null): CompletionInfo =
        client.get {
            apiUrl(SyncEndpoints.DB_COMPLETION)
    ...
    suspend fun getFolderStatus(folder: String): FolderStatus =
        client.get {
            apiUrl(SyncEndpoints.DB_STATUS)
            parameter("folder", folder)
        }.body()
    ...
    suspend fun getIgnores(folder: String): JsonArray =
        client.get {
            apiUrl(SyncEndpoints.DB_IGNORES)
            parameter("folder", folder)
        }.body()
    ...
    suspend fun overrideFolder(folder: String): Boolean {
        val response = client.post {
            apiUrl(SyncEndpoints.DB_OVERRIDE)
            parameter("folder", folder)
        }
        return response.status.isSuccess()
    }
    ...
    suspend fun revertFolder(folder: String): Boolean {
        val response = client.post {
            apiUrl(SyncEndpoints.DB_REVERT)
            parameter("folder", folder)
        }
        return response.status.isSuccess()
    }
    ...
    suspend fun getPendingDevices(): List<PendingDevice> =
        client.get { apiUrl(SyncEndpoints.PENDING_DEVICES) }.body()

    suspend fun getPendingFolders(): List<PendingFolder> =
        client.get { apiUrl(SyncEndpoints.PENDING_FOLDERS) }.body()

    // ---------- Stats ----------

    suspend fun getDeviceStats(): Map<String, DeviceStats> =
        client.get { apiUrl(SyncEndpoints.STATS_DEVICE) }.body()

    // ---------- Events ----------

    suspend fun getEvents(since: Long = 0L, limit: Int = 0): List<SyncthingEvent> =
        client.get {
            apiUrl(SyncEndpoints.EVENTS)
    ...
    suspend fun getDiskEvents(since: Long = 0L, limit: Int = 0): List<SyncthingEvent> =
        client.get {
            apiUrl(SyncEndpoints.EVENTS_DISK)
            if (since > 0) parameter("since", since)
            if (limit > 0) parameter("limit", limit)
        }.body()
    ...
    override suspend fun getFolders(): List<Folder> =
        client.get { apiUrl(SyncEndpoints.CONFIG_FOLDERS) }.body()

    /** 批量替换所有 folders 配置 */
    override suspend fun putFolders(folders: List<Folder>): Boolean {
        val response = client.put {
            apiUrl(SyncEndpoints.CONFIG_FOLDERS)
            setBody(folders)
        }
        return response.status.isSuccess()
    }
    ...
    override suspend fun getFolder(id: String): Folder =
        client.get { apiUrl(SyncEndpoints.CONFIG_FOLDER_ID + id) }.body()

    /** 替换单个 folder 配置 */
    override suspend fun putFolder(id: String, folder: Folder): Boolean {
        val response = client.put {
            apiUrl(SyncEndpoints.CONFIG_FOLDER_ID + id)
            setBody(folder)
        }
        return response.status.isSuccess()
    }
    ...
    override suspend fun getDevices(): List<Device> =
        client.get { apiUrl(SyncEndpoints.CONFIG_DEVICES) }.body()

    /** 批量替换所有 devices 配置 */
    override suspend fun putDevices(devices: List<Device>): Boolean {
        val response = client.put {
            apiUrl(SyncEndpoints.CONFIG_DEVICES)
            setBody(devices)
        }
        return response.status.isSuccess()
    }
    ...
    override suspend fun getDevice(id: String): Device =
        client.get { apiUrl(SyncEndpoints.CONFIG_DEVICE_ID + id) }.body()

    /** 替换单个 device 配置 */
    override suspend fun putDevice(id: String, device: Device): Boolean {
        val response = client.put {
            apiUrl(SyncEndpoints.CONFIG_DEVICE_ID + id)
            setBody(device)
        }
        return response.status.isSuccess()
    }
    ...
    override suspend fun getOptions(): Options =
        client.get { apiUrl(SyncEndpoints.CONFIG_OPTIONS) }.body()

    /** 替换 options 配置 */
    override suspend fun putOptions(options: Options): Boolean {
        val response = client.put {
            apiUrl(SyncEndpoints.CONFIG_OPTIONS)
            setBody(options)
        }
        return response.status.isSuccess()
    }
    ...
    override suspend fun getDefaultFolder(): Folder =
        client.get { apiUrl(SyncEndpoints.CONFIG_DEFAULTS_FOLDER) }.body()

    /** 替换默认 folder 配置模板 */
    override suspend fun putDefaultFolder(defaults: Folder): Boolean {
        val response = client.put {
            apiUrl(SyncEndpoints.CONFIG_DEFAULTS_FOLDER)
            setBody(defaults)
        }
        return response.status.isSuccess()
    }
    ...
    override suspend fun getDefaultDevice(): Device =
        client.get { apiUrl(SyncEndpoints.CONFIG_DEFAULTS_DEVICE) }.body()

    /** 替换默认 device 配置模板 */
    override suspend fun putDefaultDevice(defaults: Device): Boolean {
        val response = client.put {
            apiUrl(SyncEndpoints.CONFIG_DEFAULTS_DEVICE)
            setBody(defaults)
        }
        return response.status.isSuccess()
    }
    ...
    override suspend fun getDefaultIgnores(): List<String> =
        client.get { apiUrl(SyncEndpoints.CONFIG_DEFAULTS_IGNORES) }
    ...
    override suspend fun putDefaultIgnores(lines: List<String>): Boolean {
        val response = client.put {
            apiUrl(SyncEndpoints.CONFIG_DEFAULTS_IGNORES)
    ...
    override suspend fun getGui(): Gui =
        client.get { apiUrl(SyncEndpoints.CONFIG_GUI) }.body()

    /** 替换 gui 配置 */
    override suspend fun putGui(gui: Gui): Boolean {
        val response = client.put {
            apiUrl(SyncEndpoints.CONFIG_GUI)
            setBody(gui)
        }
        return response.status.isSuccess()
    }
